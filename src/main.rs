...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for symbol in symbols {
                ...
                (symbol["symbol"].as_str(), symbol["quoteAsset"].as_str())
            {
                if quote_asset == "ETH" {
                    ...
                    coins.push(format!("BINANCE:{}ETH", base_asset));
                }
            }
        }
    }
    ...
    if coins.is_empty() {
        println!("ETH spot piyasa çiftleri bulunamadı. API yanıtını kontrol edin.");
        return Ok(());
    }

    coins.sort_by(|a, b| {
        let a_name = a.split(':').nth(1).unwrap_or("").trim_end_matches("ETH");
        let b_name = b.split(':').nth(1).unwrap_or("").trim_end_matches("ETH");
    ...
    let mut file = File::create("binance_eth_markets.txt")?;
    for coin in &coins {
        writeln!(file, "{}", coin)?;
    }

    println!("Binance ETH spot piyasası coin listesi başarıyla oluşturuldu ve kaydedildi.");
    println!("Toplam {} adet ETH spot piyasa çifti bulundu.", coins.len());
